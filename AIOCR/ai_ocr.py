...
class Api:
    def __init__(self, globalArgd):
        ...
        logger.debug(f"AI OCR 插件初始化完成，使用服务商: {globalArgd['provider']}")
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.debug(f"原始响应:【{parsed_content}】")
                    if parsed_content:
                        # 转换为Umi格式
                        return self._convert_to_umi_format(self._parse_result(parsed_content), config)
                    else:
